...
def test_read_polygon_mesh():
	...
	max_elem = max(max(F))
	assert max_elem < V.shape[0], "There is a face with an out-of-bounds vertex index. Faces should be zero-based arrays of indices into vertices."
	...
	min_elem = np.amin(F)
	assert max_elem < V.shape[0], "There is a face with an out-of-bounds vertex index. Faces should be zero-based arrays of indices into vertices."
	assert min_elem >= 0, "There is a face with a negative vertex index. Faces should be zero-based arrays of indices into vertices."

def test_write_surface_mesh():
	V, F = read_polygon_mesh(os.path.join(asset_path, "bunny_small.obj"))
...
class TestTetSolver:
	...
	def test_get_vertices(self):
		...
		phi = solver.compute_distance_to_mesh(V, F, rebuild=True)
		...
		assert vertices.shape[0] == len(phi), "SDF should have same length as number of vertices in the tet mesh domain."

	def test_get_tets(self):
		V, F = read_polygon_mesh(os.path.join(asset_path, "bunny_small.obj"))
		solver = shm.SignedHeatTetSolver(verbose=False)
		phi = solver.compute_distance_to_mesh(V, F, rebuild=True)
		vertices = solver.get_vertices()
		...
		assert max_elem < vertices.shape[0], "There is a tet with an out-of-bounds vertex index. Tets should be zero-based arrays of indices into vertices."
		assert min_elem >= 0, "There is a tet with a negative vertex index. Tets should be zero-based arrays of indices into vertices."

	def test_compute_distance_to_mesh(self):
		V, F = read_polygon_mesh(os.path.join(asset_path, "bunny_small.obj"))
		solver = shm.SignedHeatTetSolver(verbose=False)
		phi = solver.compute_distance_to_mesh(V, F, rebuild=True)
	...
	def test_compute_distance_to_point_cloud(self):
		P, N = read_point_cloud(os.path.join(asset_path, "bunny.pc"))
		solver = shm.SignedHeatTetSolver(verbose=False)
		phi = solver.compute_distance_to_point_cloud(P, N, rebuild=True)
		assert len(phi.shape) == 1, "SDF should be a 1D NumPy array."
	...
	def average_squared_distance(self, V1: np.ndarray, F1: list[list[int]], V2: np.ndarray, F2: list[list[int]]) -> float:
		'''
	...
	def test_isosurface(self):
		V, F = read_polygon_mesh(os.path.join(asset_path, "bunny_small.obj"))
		solver = shm.SignedHeatTetSolver(verbose=False)
		phi = solver.compute_distance_to_mesh(V, F, rebuild=True, level_set_constraint="ZeroSet")
		V_iso, F_iso = solver.isosurface(phi, 0.)
...
class TestGridSolver:
	...
	def test_get_grid_resolution(self):
		V, F = read_polygon_mesh(os.path.join(asset_path, "bunny_small.obj"))
	...
	def test_get_bbox(self):
		...
		assert not np.any((bbox_max - bbox_min) < 0), "Grid corners should be returned in order of minimal -> maximal node corner."

	def test_to_grid_array(self):
		V, F = read_polygon_mesh(os.path.join(asset_path, "bunny_small.obj"))
		solver = shm.SignedHeatGridSolver(verbose=False)
		phi = solver.compute_distance_to_mesh(V, F, rebuild=True)
	...
	def test_compute_distance_to_mesh(self):
		V, F = read_polygon_mesh(os.path.join(asset_path, "bunny_small.obj"))
		solver = shm.SignedHeatGridSolver(verbose=False)
		phi = solver.compute_distance_to_mesh(V, F, rebuild=True, h_coef=2.)
		assert len(phi.shape) == 1, "SDF should be a 1D NumPy array."
		# Make sure distance is close to naive distance.
	...
	def test_compute_distance_to_point_cloud(self):
		P, N = read_point_cloud(os.path.join(asset_path, "bunny.pc"))
		solver = shm.SignedHeatGridSolver(verbose=False)
		phi = solver.compute_distance_to_point_cloud(P, N, rebuild=True, h_coef=2.)
		assert len(phi.shape) == 1, "SDF should be a 1D NumPy array."
		# Make sure distance is close to naive distance.
		nx, ny, nz = solver.get_grid_resolution()
		bbox_min, bbox_max = solver.get_bbox()
